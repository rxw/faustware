...
How long would this take? Looking from the back forward is simpler: row 33 takes 0 time (everything comes to them). Row 32 only has to walk to row 33, so that’s \(6 \times 10 \times 1 = 60\) seconds, and so on. The total time in seconds looks like:
...
= 60 \cdot 528
= 31{,}680 \text{ seconds}
\approx 528 \text{ minutes} \approx 8.8 \text{ hours}.
\]
...
But since the front and back traverse these distances simultaneously, the effective traversal time is cut in half, giving only **128 row-traversals’ worth of time**. At ~1 minute per row (6 seats × 10 s), that’s:

\[
128 \text{ minutes} \approx 2\text{ hours 8 minutes}.
\]

Ample time for everyone to trick-or-treat after takeoff and before starting our final descent.
